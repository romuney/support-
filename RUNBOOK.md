...
из `GET /api/v4/users/me` тем же credential при каждом прогоне. Раньше это
была переменная `BOT_USER_ID`, которую негде было взять: в интерфейсе id
не показан, а Bot Accounts отдаёт Token ID той же формы. Реакция ставится
в личке на сообщение человека и в канале — только на сообщение с тегом;
на посты формы в `hr-report-ask` нет: там реакции — словарь дежурного.

**Реакций две, и они сменяют друг друга.** `:bully_work:` (GIF) встаёт, как
только бот взялся; после того как ответ ушёл, она снимается и на то же
сообщение встаёт `:bully_ready:`. Так по одному взгляду видно состояние:
GIF — работает, гордая поза — ответил, ничего — не услышал. Пак эмодзи —
`иконки/`, залит в Mattermost под именами файлов из `иконки/out/`; нет
эмодзи на сервере — узел реакции падает мягко, ответ уходит всё равно.
Ими же размечены шапка и разбор в канале джуна (светофор `:bully_high:` /
`:bully_mid:` / `:bully_none:` / `:bully_unknown:`, маркеры `:bully_flag:`,
`:bully_warn:`, `:bully_lock:`, `:bully_expert:`); в тексте, который уходит
заказчику, остаётся юникод — за пределами Time наши эмодзи не рисуются.
...
node test_recon.mjs      # разведка DD: формы ответов каталога, ключи ссылок отчётов
```
...
### Два канала в одном фильтре триггера — не работает. Подтверждено

02.09: с двумя каналами в `postedFilters` одного `Time Trigger` прогонов
не было девять часов при статусе Published. Владелец удалил один канал
из фильтра — события пошли сразу. Узел кастомный, схемы нет; как он
регистрирует вебхуки по списку, неизвестно, и выяснять незачем: сборщик
ставит **по триггеру на канал**, каждый с одним каналом, все ведут
в `Guard channel`. Добавляя канал через `CHANNEL_LISTEN`, получаешь ещё
один триггер, а не ещё одну строку в фильтре.
...
### Git-флоу: одна ветка, `main`

**Правило и его причина — в корневом [`CLAUDE.md`](CLAUDE.md), раздел «Одна
ветка: `main`».** Здесь только порядок действий.

Работа идёт в `main`, feature-ветки не заводятся. Причина не только
в удобстве: бот в n8n читает статьи GitLab-нодами **без указания ветки** →
из ветки по умолчанию. Правка в feature-ветке для бота не существует вовсе.

1. `git checkout main && git pull origin main`.
2. `python3 validate_kb.py` перед коммитом в `kb/`; тесты — перед коммитом
   в `bot/` и `telemetry/` (список — в начале этого файла).
3. `git push origin main` — **требует подтверждения на каждый вызов**,
   в песочнице push по SSH недоступен, используется HTTPS remote.
4. Push отклонён — `git pull --rebase origin main`, прогнать тесты заново
   и запушить. Ветку «чтобы не мешать» не заводить.

Запреты те же, что были: `push --force`, `reset --hard` по запушенному,
`rebase` и `commit --amend` по чужим коммитам. Ошибку чинят новым коммитом.

Шаблон задания для агента — [`prompt-git-agent.md`](context/prompt-git-agent.md).
**Он написан под старый флоу с feature-веткой и MR: в этой части устарел,**
ветку из него не брать. Остальное (запреты, формат отчёта) в силе.
...
### После пуша в `main` — как проверить, что бот увидел правку
...
## 5a. Ссылка на отчёт → карточка отчёта

Половина обращений про отчёты приходит ссылкой, а не названием: в выгрузке
канала за июль–август поле «Отчет» заполнено в 47 обращениях из 156, и в 37
из них стоит ссылка. Что происходит с ней теперь:

```
ссылка в форме        →  Guard channel: report_url (до двух ссылок, юнит отсеян)
                      →  Plan: разбор ключа (REPORT_LINK_JS) + поиск в реестре,
                         таблица «## Ссылки отчётов»
                      →  добор статьи kb/reports/ + dd_urn ПЕРВЫМ объектом DD
                      →  DD Lookup: назначение, «как читать», владелец,
                         канал поддержки, где открыть, витрины-источники
                      →  Build materials: заметка «ОТЧЁТ УЗНАН ПО ССЫЛКЕ»
                      →  Author: сверить проблему с назначением, иначе —
                         владелец и канал поддержки из карточки
```

**Ключ ссылки и ключ объекта в каталоге не связаны ничем.** У «Активности
в GitLab» ключ дашборда `35005`, а в DD она `aktivnost-v-gitlab`; у «Детальных
списков» наоборот. Онлайн мост не строится: поиск по ключу ссылки отдаёт пусто,
по числовому ключу — чужую таблицу, а от витрины к дашборду в каталоге пути нет
(`context/DD-REPORT-CARDS.md`, § 1 и § 6). Поэтому пара живёт в git.

**Три исхода, и они разные.** `resolved` — отчёт узнан; `permalink` — прислали
`…/dashboard/p/…`, адрес сохранённого состояния дашборда, по нему отчёт
не определяется ничем, и бот просит ссылку из адресной строки; `unknown` —
ключ есть, строки моста нет, и джун получает готовую задачу «завести строку».
Исход и ключ стоят в трассе, в служебном блоке джуну и в телеметрии
(`report_link`, `report_link_key`).

### Как наполнить мост

Строки печатает разведка, руками их не выписывают:

1. Импортировать `bot/DD Recon.json`, запустить (ручной триггер).
2. Открыть ноду **«Shape report keys»** — там готовые строки
   `| ключ | id отчёта | проверено |` по всем отчётам реестра, плюс три
   отдельных списка: расхождение со ссылкой из статьи (заводить ОБЕ строки —
   у отчёта бывает и числовой ключ, и слаг), пермалинк в карточке DD
   (в мост не идёт, чинится владельцем отчёта в DD) и «ссылки нет».
3. Вставить в `executive-support/kb/index.md`, таблица «Ссылки отчётов»,
   проставить дату в колонке «проверено».
4. `cd executive-support && python3 validate_kb.py`.

Валидатор ловит: ключ, назначенный двум отчётам; ключ со ссылкой целиком
вместо голого хвоста; пермалинк в мосте; id не типа `report`; и ключ
из `links:` статьи, не заведённый в таблицу.
...
- [x] Заполнить `BOT_USER_IDS` в сборщике телеметрии — id из трассы ядра
      02.09 (`Who am I`), после правки переимпортировать `Telemetry Ingest`
- [ ] Заполнить `DM_ALLOWLIST` в сборщике бота
- [ ] Подтвердить живым запуском `Adapter DM` и URN оставшихся 4 таблиц
- [ ] Собрать дашборд в Proteus поверх `support_request`
- [ ] Прогнать фазу K разведки и наполнить таблицу «Ссылки отчётов»:
      сейчас 13 ключей на 13 из 15 отчётов, взяты из `links:` статей
      и из FEEDBACK_KEYS, живым запросом не сверялись (колонка
      «проверено» — прочерк). Без строки: r-avatar-center (Tableau)
      и r-social-graph (ссылка на карточку каталога)
