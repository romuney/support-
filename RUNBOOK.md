...
**После импорта включить workflow.** Файлы приезжают с `active: false`,
и пока адаптер выключен, триггер Mattermost не срабатывает вовсе —
Executions остаётся пустым, что неотличимо от «бота позвали, а он молчит».
Первое, что проверять при тишине: тумблер Active у `Adapter Channel`
и `Adapter DM`.
