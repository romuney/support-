...
за деталями». Все архитектурные решения и их причины — в [`AGENTS.md`](AGENTS.md)
...
Полная карта решений: [`AGENTS.md` — «Флоу бота: git + DD в один контекст»](AGENTS.md),
[«Подключение к Time: ядро и три адаптера»](AGENTS.md), [«Телеметрия канала»](AGENTS.md).
...
[`prompt-git-agent.md`](prompt-git-agent.md): содержит текущую ветку, что нельзя
