...
|---|---|---|
| `BOT_USERNAME` | имена, на которые бот отзывается в канале: логин латиницей И отображаемое имя, через запятую | путь по @упоминанию выключен целиком — бота зовут, он молчит |
...
```bash
BOT_USERNAME=bully,Булли BOT_USER_ID=<id> python3 build_time_flows.py
```

**Имён у бота два, и нужны оба.** Кириллица в username Mattermost запрещена,
поэтому логин латиницей, а зовут его люди так, как видят в клиенте, —
«@Булли». Одно значение здесь означает выбрать, какое написание работает,
а какое молча нет.
