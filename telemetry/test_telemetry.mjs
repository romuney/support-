...
line('37. Витрина знает про КАЖДОЕ поле ответа бота — или называет, почему нет');
{
  ...
  // НОД ДВЕ, А НЕ ОДНА. Поля ответа считаются и в «Parse answer», и в
  // «Final answer» — второй нодой, в частности, считаются ВСЕ проверки
  // черновика, которым нужен разрешённый ключ юнита и результат проверки
  // значений. Пока тест читал только первую, draft_asks_date,
  // draft_foreign_ids, draft_key_unlabeled, check_retried и
  // check_exact_values не были покрыты ничем: код их считал, витрина
  // не читала, и блок неиспользуемых полей о них молчал. То есть проверка
  // «ни одно поле не потеряно» теряла поля целой нодой — ровно тот же
  // отказ, что она сама и ловит, и уже второй раз (первый был про отступ
  // в якоре, из-за которого мимо проходили присваивания внутри блоков).
  const NODES = ['Parse answer', 'Final answer'];
  const fields = new Set();
  for (const nm of NODES) {
    const node = CORE.nodes.find((n) => n.name === nm);
    check(`нода «${nm}» найдена`, Boolean(node));
    if (!node) continue;
    ...
    // experts_invented и draft_own_tools проскакивали мимо теста.
    const own = [...node.parameters.jsCode.matchAll(/^\s*out\.([a-z_0-9]+)\s*=/gm)]
      .map((m) => m[1]);
    check(`  «${nm}»: полей найдено ${own.length}`, own.length > 0);
    for (const f of own) fields.add(f);
  }
  // Число печатается намеренно: зелёный тест на сломанном детекторе и на
  // чистом коде выглядит одинаково, а вот резкое падение числа сразу видно.
  check(`полей на выходе ядра (${fields.size})`, fields.size >= 25);
  ...
  const inMart = (f) => SQL.includes(`'$.${f}'`) || new RegExp(`max_by\\(${f},`).test(SQL);
  // САМОПРОВЕРКА ДЕТЕКТОРА. Зелёный тест на сломанном детекторе и на чистом
  // коде выглядит одинаково: и там, и там ни одной красной строки. Поэтому
  // перед прогоном по настоящим полям кормим ему заведомо потерянное имя —
  // не найдётся ни в витрине, ни в блоке причин. Не покраснело здесь —
  // значит ниже зелень ничего не значит.
  const CANARY = 'zzz_field_that_must_not_exist';
  check('сам детектор рабочий: выдуманное поле не проходит',
    !inMart(CANARY) && !named.has(CANARY));
