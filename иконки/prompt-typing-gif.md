...
```
python3 make_gif.py typing-sheet.jpg --out out/bulli_typing.gif --ms 80
```

Девять кадров по 80 мс = цикл 0.72 с, темп «быстро печатает». Медленнее
120 мс читается уже как «тыкает одним пальцем».

**Без `--align` и `--freeze-top` намеренно.** Оба ключа глушат ровно то,
ради чего анимация и делается: `--align` двигает кадры так, чтобы голова
совпала с первой, то есть гасит КИВОК; `--freeze-top` пришивает кепку
и основания ушей, то есть гасит МОТАНИЕ УШАМИ. Решение по этому боту —
пусть болтается: непостоянство формы читается как энергия, а не как брак.

Ключи оставлены для случая, когда лист вышел действительно дрожащим —
например если поехала клавиатура или масштаб головы гуляет так, что цикл
рябит. Тогда сначала `--align`, и только если не хватило — `--freeze-top`.
Порядок именно такой: совмещение ничего не стирает, заморозка стирает.

## Пиксельного совмещения кадров от модели ДОБИТЬСЯ НЕ УДАЛОСЬ

Прогон 2026-09-01 дал лист, где стиль верный и камера по силуэту держится
отлично: ширина 303–305 px при ячейке 341, низ 323–327, разброс сверху —
это ровно поднятая лапа. По внешним габаритам не придраться.

А замер того, что двигаться НЕ должно, показал другое: голова и кепка
отличаются между соседними кадрами на **17–62 из 255**, клавиатура на 17–41.
При настоящей фиксации там были бы единицы. То есть модель не двигает лапы
на неизменном кадре — она **рисует каждую ячейку заново**, просто похоже.

Совмещение сдвигом снимает меньше половины: 46 → 28 в среднем. Остальное
не сдвиг, а перерисовка — другая форма ушей, другой масштаб головы, в одном
кадре уши вообще висячие. Сдвигом это не лечится по определению.

Короткий цикл тоже не спасает: из 36 пар кадров **только одна** (2–3) держит
голову ровно и при этом показывает движение лап. Лучшая четвёрка (1-4-6-7)
даёт худший стык 25 из 255.

**Заморозка верха кадра это лечит**, но ценой самого движения — см. выше,
для этого бота приём не применяется. Замер приведён на случай, когда цикл
всё же придётся успокаивать. Кепка с надписью берётся
из первого кадра во все остальные: расхождение в этом окне падает
с 16–47 до 0.6–2.8, шва не видно, а выпадающий кадр с висячим ухом
подтягивается к остальным. Глаз цепляется за текст сильнее всего, поэтому
неподвижная надпись CROSS убирает бо́льшую часть заметного дрожания,
хотя морда под ней продолжает меняться.

Окно заморозки — 20–80 % ширины и верхние 30 % высоты, **уже окна
совмещения**: поднятые лапы туда не дотягиваются, иначе замороженная кепка
затирала бы движение. Проверяется прогоном по доле непрозрачного в окне,
а не на глаз.

### Если нужен цикл без дрожания вовсе

Лист как способ исчерпан: девять ячеек — это девять генераций внутри одной
картинки, и пиксельного совпадения между ними не будет. Другой путь —
**править ОДИН принятый кадр**, по кадру за прогон:

1. взять с этого листа лучший кадр, вырезать его отдельным PNG;
2. приложить ЕГО (а не `bulli-ref.png`) и попросить:

```
Keep every single pixel of the attached image identical: the same head, the
same cap, the same ears, the same eyes, the same keyboard, the same
background, the same position and the same size. Change ONLY the front paws:
<движение>. Everything else must stay exactly as it is.
```

3. собрать с `--align --freeze-top 0.30`.

Режим правки сохраняет нетронутое несравнимо лучше, чем рисование девяти
ячеек с нуля, — ровно как это вышло со стилем стикеров. Цена: восемь
прогонов вместо одного.
