...
def main():
    ...
    ap.add_argument("--inset", type=float, default=1.5, metavar="ПРОЦЕНТ",
                    help="срезать столько процентов с каждого края ячейки "
                         "перед вырезанием фона: заливка идёт от края, и "
                         "любой остаток разделителя её останавливает")
    a = ap.parse_args()

    cells = sheet.split_grid(Image.open(a.image), a.cols, a.rows)

    # Разделитель, попавший в ячейку, останавливает заливку фона, и на выходе
    # получается не прозрачный PNG, а серый квадрат с персонажем. `find_gaps`
    # ловит полосы, только когда они идут через весь лист; на листе 3x2 из
    # прогона 07.09 плашки перекрыли белые линии, доля яркого не дотянула
    # до порога, деление ушло в ровное — и две ячейки из шести приехали
    # с фоном. Срезать край дешевле, чем угадывать цвет полосы: персонаж
    # к самому краю ячейки не подходит, там по заданию поле.
    if a.inset:
        cells = [c.crop((round(c.size[0] * a.inset / 100),
                         round(c.size[1] * a.inset / 100),
                         round(c.size[0] * (1 - a.inset / 100)),
                         round(c.size[1] * (1 - a.inset / 100))))
                 for c in cells]
