...
> нужно [`prompt-each.md`](prompt-each.md): там тот же состав и те же имена,
> но под персонажа встаёт круглая плашка, а обратная связь стала лапой
> без плашки. Причины переделки — в разделе «Что не так с v1»
> в [`prompt-pack-v2.md`](prompt-pack-v2.md), который тоже не запускается
> и остаётся разбором. Здесь остаётся разбор прогона v1 и **таблица «Где это
> живёт в коде», которая верна по-прежнему**: имена эмодзи v2 не меняет.
