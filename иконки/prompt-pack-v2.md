# Разбор: пак v2 листом 4×4 — диск с лучами (не запускается)

> **Это задание не прогонялось и запускать его не надо.** Живое задание пака
> одно — [`prompt-each.md`](prompt-each.md), по картинке на иконку; пак
> `out-badge/` пришёл оттуда. Здесь остаётся разбор: почему v1 не читался
> в плашке реакции, замеры палитры и почему диск не может быть серым.
> Всё это верно и в поштучной форме.
>
> **Разъехались эти два файла молча, и стоило это пака.** Оба описывали v2,
> но по-разному: здесь — плоское солнце с треугольными лучами наружу, диск
> в 62 % ячейки, предметы белые; в `prompt-each.md` — круглая плашка
> в девять десятых кадра, мягкий градиент, размытые лучи внутри, предметы
> своего цвета. README при этом называл «тем, что запускать», **этот** файл,
> а прогон 06.09 шёл по тому. Отсюда правило: **живая спецификация одна.**
> Композиция здесь оставлена как была — она нужна для разбора; за тем, как
> пак выглядит сейчас, ходить в `prompt-each.md`.
...
Нарезка тем же `slice_grid.py` и тем же `names-c.txt`. **Код трогать
не нужно**: имена эмодзи не меняются.
