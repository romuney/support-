...
| employee-directories | должность, позиция сотрудника, профессия, специализация, грейд, стрим, направление, табельный номер, мастер ID, MasterID, идентификатор сотрудника, ID сотрудника, покраска, HQ, BigOps, логин, ad_login, TWork, связь с клиентом, справочник атрибутов сотрудника | t-emp-structure, rc-field-synonyms |
...
| allocation | продукт, продуктовая структура, аллокация, FTE, загрузка на продукт, функциональная роль, КП, каталог продуктов, уровень КП, юнит КП, руководитель продукта, лид продукта, владелец продукта | t-functional-unit, m-fte-product |
...
| evaluations | оценки, ревью, годовая оценка, итоговая оценка, оценка сотрудника, перформанс, оценочные конференции, ОК РС, оценка руководителей секторов | t-summary-evaluation |
| discipline | дисциплинарные взыскания, взыскание, выговор, замечание, нарушение дисциплины, дисциплинарка | t-disciplinary-sanction |
| residence | город проживания, место жительства, где живёт сотрудник, локация сотрудника, регион проживания, город сотрудника | t-employee-residence |
| family | дети, дети сотрудников, сотрудники с детьми, ребёнок, многодетные, семья сотрудника | t-employee-children |
| statements | заявления сотрудников, поля заявлений, содержимое заявления, что написано в заявлении, справки, возврат трудовой книжки, БТК, бумажная трудовая книжка, ЭТК | t-statement-field |
...
Домен дробится по двум причинам, и они разные.

Первая — **размер**: сущностей в нём стало больше десяти **не считая мастеров**.
Мастера не выбирают, их добирает код по этой таблице, и на сложность выбора они
не влияют. До порога дробление не сужает поиск, а только добавляет поводов
ошибиться при выборе.

Вторая — **мастер не покрывает тему**. Домен обещает тему в колонке «о чём
вопросы», а его мастер про эту тему ничего не знает: попав в домен верно, код
не добирает ничего, и всё держится на том, назовёт ли роутер нужную строку
поимённо. Выбрать одну строку из полусотни модель повторяемо не умеет —
значит домен, где темы и мастера разошлись, работает через раз независимо
от размера. Валидатор ловит два частных случая: мастера без статьи и домен,
у которого единственный мастер — витрина по умолчанию (она читается на каждом
вопросе и так, то есть не добавляет ничего).
...
**`evaluations`, `discipline`, `residence`, `family` и `statements` — это бывший
`personal-attributes`, разделённый 2026-09-02.** Сущностей в нём было десять,
то есть до порога размера он не дотягивал, — но дробить пришлось по второй
причине. Домен обещал шесть разных тем (оценки, взыскания, город проживания,
дети, заявления), а мастером у него стояла витрина сотрудников, где нет ни
одной из них. Каждая тема лежала своей таблицей, и ни одна мастером
не объявлялась: попав в домен верно, код не читал ничего.

Живой прогон 02.09 — «где мне взять инвалидов, у которых есть дети» — тот же
вопрос то решался с первого захода, то нет: всё зависело от того, назовёт ли
роутер `t-employee-children` поимённо. Теперь у каждой темы свой мастер,
и достаточно попасть в тему.

Отчёты домена разошлись по смыслу: `r-ok-rs` (оценочные конференции) —
в `evaluations`, `r-btk-location` (заявления на возврат бумажной трудовой
книжки) — в `statements`, `r-ambassadorstvo` и `r-gitlab-activity` (время
на внепроектные активности) — в `attendance`, `r-referral-support`
(программа «Приведи друга») — в `movement`.
...
| r-ambassadorstvo | report | attendance | Амбассадорство сотрудников | kb/reports/ambassadorstvo.md | urn:dd:reports:reports:report:ambassadorstvo-vneproektnye-aktivnosti-sotrudnikov | амбассадорство, амбассадоры, программа амбассадоры, участие в амбассадорстве, амбассадорство сотрудников | active | Отчёт об участии сотрудников в программе Амбассадоры: время на активности, частота участия и доля участвующих от численности. RLS |
...
| r-gitlab-activity | report | attendance | Активность в GitLab | kb/reports/gitlab-activity.md | urn:dd:reports:reports:report:aktivnost-v-gitlab | активность в gitlab, gitlab активность, активность в git, метрики gitlab, активность разработчиков в gitlab | active | Метрики активности сотрудников в GitLab. RLS |
...
| r-ok-rs | report | evaluations | Отчет по ОК РС | kb/reports/ok-rs.md | urn:dd:reports:reports:report:1804 | отчет по ок рс, ок рс, оценочные конференции руководителей секторов, оценка руководителей секторов, rs assessment | active | Результаты оценочных конференций Руководителей Секторов. RLS |
...
| r-referral-support | report | movement | Поддержка программы Приведи друга | kb/reports/referral-support.md | urn:dd:reports:reports:report:2048 | приведи друга, поддержка программы приведи друга, запросы в поддержку приведи друга, referral support, cross sd поддержка программы приведи друга | active | Запросы в поддержку программы Приведи друга из чата MyT и Forge, аналитика производительности и SLA. RLS |
| r-social-graph | report | org-structure | Социальный граф | kb/reports/social-graph.md | urn:dd:reports:reports:report:2668 | социальный граф, социальный граф юнита, метрики социального графа, граф связей сотрудников, связи сотрудников в юните, соц граф | active | Метрики по социальному графу юнита в управленческой структуре. RLS для сокрытия чувствительных данных |
...
| rc-field-synonyms | recipe | * | Словарь синонимов | kb/recipes/field-synonyms.md | — | словарь, синонимы, как называется поле, стрим, HQ, какое поле брать, специализация, грейд, покраска, BigOps, Big Ops, Non-HQ, Line, Support, IT, Digital, Non-IT, мастер ID, MasterID, идентификатор сотрудника, табельный номер | active | Слово заказчика в имя поля: покраска HQ/Line/Support и IT/Digital/Non-IT, 13 значений active_type_nm, мастер ID (mdm_employee_rk) против табельного номера (employee_main_work_no) |
...
| rc-unit-link | recipe | org-structure, allocation | Ссылка на юнит — как превратить её в ключ витрины | kb/recipes/unit-link.md | — | ссылка на юнит, id юнита, идентификатор юнита из ссылки, my.tbank.ru, hr.tbank.ru, ссылка на подразделение, ссылка на продукт, uuid юнита, как найти юнит по ссылке | active | Ссылка → id → rk → основные витрины. Вид ссылки определяет структуру: /structure/resource/units/ — управленческая, /product-catalog/product/ — Каталог продуктов |
...
| t-emp-structure | table | headcount, org-structure, employee-directories, movement | Ультраширокая витрина сотрудников | kb/tables/mdm-employee-structure-d.md | urn:dd:tables:greenplum:table:emart.mdm_employee_structure_d | mdm_employee_structure_d, ультраширокая, витрина сотрудников, основная витрина, в штате, штатный, emp_structure, тип трудоустройства, форма занятости, employment_relation_type_desc, тип договора, формат работы, employee_contract_type_desc, гибрид или офис | active | Гранулярность: сотрудник × день, непрерывная. Основной источник. Тип трудовых отношений — employment_relation_type_desc (Штат/ГПД/Аутсорс), формат работы — employee_contract_type_desc (гибрид/офис/дистант) |
| t-employee-children | table | family | Данные о детях сотрудников | kb/tables/employee-children.md | urn:dd:tables:greenplum:table:chrono_peoplehub_masterid.individualchildren_public | individualchildren_public, дети, дети сотрудников, ребёнок, дети сотрудника, данные о детях сотрудников, сотрудники с детьми, многодетные | active | ПДн высокой чувствительности. Обязателен фильтр isdeleted = false и birthdate <= fire_dt |
...
| t-employee-residence | table | residence | Город проживания сотрудника | kb/tables/employee-residence.md | urn:dd:tables:greenplum:table:sse_crossdata.employee_residence | employee_residence, город проживания, город отправки документов, пребывание сотрудника, мастер локаций, локации сотрудника, city_nm | active | Мастер локаций сотрудника: город проживания (city_nm) и город отправки документов. Джойн с t-emp-structure по mdm_employee_rk + valid_to_dttm='5999-01-01'. Доступ закрыт группами, требуется согласование |
...
| t-head-segments-versions | table | org-structure | Сегменты руководителей структуры команды | kb/tables/head-segments-versions.md | urn:dd:tables:greenplum:table:usr_cross_data.head_segments_versions | head_segments_versions, usr_cross_data.head_segments_versions, сегмент руководителя, сегменты структуры команды, head_lvl_segment | active | Версионная витрина сегментов структуры команды; сегмент руководителя — поле head_lvl_segment. Джойн с t-emp-structure по mdm_employee_rk + valid_to_dttm='5999-01-01' |
...
| t-mdm-employee | table | employee-directories | Единый сотрудник (актуальные данные) | kb/tables/mdm-employee.md | urn:dd:tables:greenplum:table:emart.mdm_employee | mdm_employee, emart.mdm_employee, единый сотрудник, актуальные данные сотрудника, сотрудник на стадии оформления, оформление сотрудника, сотрудники т-банка | active | Мастер-справочник сотрудников Т-банка, включает оформляемых. Джойн с t-emp-structure по mdm_employee_rk. Для численности — не сюда, а в t-emp-structure |
...
**Мост «ключ ссылки Proteus → отчёт».** Половина обращений про отчёты приходит
ссылкой, а не названием: в выгрузке канала за июль–август поле «Отчет» заполнено
в 47 обращениях из 156, и в 37 из них стоит ссылка. Без этой таблицы бот видит
ссылку и не знает, какой это отчёт: `report_url` работал признаком «спрашивают
про отчёт» и больше ничем.

**Вывести ключ из `dd_urn` нельзя, и это проверено.** Ключ ссылки и id в URN
не связаны никак: у «Активности в GitLab» URN `report:aktivnost-v-gitlab`,
а ссылка `/dashboard/35005/`; у «Детальных списков» наоборот — URN `report:1728`,
ссылка `/dashboard/hr-executive-detail-employee`. Онлайн мост тоже не строится:
`POST /search/query` по ключу ссылки отдаёт пусто, по числовому ключу — чужую
таблицу, а от витрины к дашборду в каталоге пути нет ни при каком лимите
(разбор — `../../context/DD-REPORT-CARDS.md`, § 1 и § 6). Поэтому пара живёт
в git: она единственная не протухает — меняется то, что лежит по URN,
а не сама пара.

Отдельная таблица, а не колонка в «Сущности», по двум причинам. У одного отчёта
ключей НЕСКОЛЬКО — числовой, слаг и старый слаг ведут на один дашборд, —
и в одной ячейке они превратились бы в список, который надо парсить. И роутеру
эти ключи не нужны вовсе: он выбирает статьи по смыслу, а ключ резолвит код.
Таблица вырезается из промпта роутера так же, как «Самостоятельные выгрузки»
и «Маршруты» (`CODE_ONLY` в узле «Decode registry»), и стоит ноль токенов.

`ключ` — то, что стоит в ссылке после `/superset/dashboard/`, без слэшей,
query и якоря. Хост не хранится: тот же дашборд отдаётся и с `proteus.tcsbank.ru`,
и с `proteus.tbank.ru`, а признак «наш хост» проверяет код — ровно так же
устроена ссылка на юнит. Вид ключа (числовой или слаг) тоже не хранится:
он виден из самого ключа, а вторая копия признака разъехалась бы молча.

**Пермалинков здесь нет и быть не должно.** `/superset/dashboard/p/<ключ>/` —
это ссылка на СОСТОЯНИЕ дашборда, её создаёт кнопка «поделиться» на каждый
шаринг, и на один отчёт их сколько угодно. Строка на пермалинк закрыла бы
одно обращение из тысячи и притворялась бы правилом. Бот пермалинк узнаёт
и честно просит прислать ссылку из адресной строки дашборда.

`проверено` — когда пара подтверждена живым запросом `GET /entity/{urn}/link`.
Прочерк значит, что ключ взят из поля `links:` статьи: его записал человек,
и это свидетельство, а не подтверждение. Сверяет и проставляет дату фаза K
разведки — `bot/DD Recon.json`, нода «Shape report keys»: она печатает готовые
строки этой таблицы по всем отчётам реестра.

`id отчёта` обязан существовать в таблице «Сущности» с типом `report`.
Один ключ не может вести к двум отчётам — это проверяет `validate_kb.py`.

| ключ ссылки | id отчёта | проверено |
|---|---|---|
| 14586 | r-referral-support | — |
| 15657 | r-btk-location | — |
| 19710 | r-attendance-calendar | — |
| 25690 | r-online-meetings-hq | — |
| 31423 | r-employment-period | — |
| 35005 | r-gitlab-activity | — |
| 4823 | r-legal-position-period | — |
| employee-activity-detail | r-daily-activity-detail | — |
| employee_ambassadorship | r-ambassadorstvo | — |
| hr-executive-detail-employee | r-hr-detail-list | — |
| hr-executive-report | r-hr-executive-report | — |
| payments_requsites | r-payment-details | — |
| rs_assessment_report | r-ok-rs | — |

Ключ `35005` пришёл не из статьи — там стоит пермалинк, — а из фидбека
аналитика (`bot/build_dd_recon.py`, `FEEDBACK_KEYS`). Это то же свидетельство
человека, что и `links:`, и прочерк в «проверено» у него по той же причине.

Двух отчётов реестра здесь нет, и у каждого своя причина, а не общая:
`r-avatar-center` — ссылка ведёт в Tableau, а не в Proteus;
`r-social-graph` — ссылка ведёт на карточку в каталоге, а не на сам отчёт.
Оба закрывает фаза K разведки: она спросит `/link` у каталога и напечатает
готовую строку либо скажет, что ссылки в карточке нет вовсе.
