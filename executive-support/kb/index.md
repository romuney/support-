...
Куда уходит обращение, если оно **не к нам**. Половина канала — это не вопрос
про данные: 13 обращений из 49 в фидбеке аналитика за 14–26 августа решались
одной строкой «это ведёт такой-то, напиши ему». Ответа в базе на них нет
и быть не может, а бот до сих пор отвечал по витринам, потому что домен
...
Колонки: `кому` — кого тегать, `где` — канал или бот, прочерк значит «в личку»,
`проверено` — когда маршрут последний раз подтверждался практикой.

Маршрут **не заменяет ответ**. Если на вопрос есть ответ в базе — сначала
ответ, и только потом адресат. Обратный порядок превращает бота в переадресацию
и обесценивает его там, где он полезен.
