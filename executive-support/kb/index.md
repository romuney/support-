...
Трёх отчётов реестра здесь нет, и у каждого своя причина, а не общая:
`r-gitlab-activity` — в статье стоит пермалинк, ключа дашборда мы не знаем;
...
Все три закрывает фаза K разведки: она спросит `/link` у каталога и напечатает
