...
// ===================================================================== 36
line('36. МАРШРУТ печатается ДЖУНУ вместе с датой подтверждения');
{
  // Маршрут — единственный факт в служебном блоке, который код НЕ проверяет
  ...
  const ANSWER = `ЧЕРНОВИК ОТВЕТА: Квоты ведёт Kirill Seliverstov, напишите ему.
ИСТОЧНИКИ: —
УВЕРЕННОСТЬ: средняя`;
  ...
  check('строка маршрута в треде есть', thread.includes('Это ведёт не CrossData'));
  check('адресат назван', thread.includes('Kirill Seliverstov'));
