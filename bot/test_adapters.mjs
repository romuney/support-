...
line('16. Проводка адаптеров: связи и фильтры');
{
  ...
  // ПЕРЕПИСКА ТРЕДА ДОЕЗЖАЕТ ДО РОУТЕРА. Прогон 02.09 21:34: три реплики
  // и тег отдельным сообщением — роутер видел одно «@bully», сказал
  // no_question, и база не читалась вовсе. Планирует роутер, значит
  // переписку обязан видеть он, а не только автор.
  {
    const rt = router.parameters.text;
    check('роутер получает переписку треда по имени узла',
      /\$\('When called by adapter'\)\.first\(\)\.json\.thread/.test(rt));
    check('и блок переписки стоит ДО вопроса',
      rt.indexOf('=== ПЕРЕПИСКА В ТРЕДЕ ===') > 0 &&
      rt.indexOf('=== ПЕРЕПИСКА В ТРЕДЕ ===') < rt.indexOf('ВОПРОС КОЛЛЕГИ'));
    check('правило про тег без текста названо',
      /Тег без текста при непустой переписке/.test(rt));
    check('плейсхолдер переписки заполнен', !rt.includes('{{THREAD}}'));
  }
