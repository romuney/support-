...
line('ФАЗА K: КЛЮЧ ССЫЛКИ PROTEUS ПО ОТЧЁТУ РЕЕСТРА');
{
  ...
  // ТРИ колонки, дословно как в реестре: `ключ ссылки | id отчёта |
  // проверено`. Четвёртая («вид») ломала бы вставку «как есть» — валидатор
  // считает ячейки и ругается на несовпадение с шапкой, а обещание
  // «вставляется как есть» перестаёт быть правдой.
  check('готовая строка печатается в формате таблицы реестра',
    /\| hr-executive-detail-employee \| r-hr-detail-list \| /.test(out));
  check('и шапка блока — та же, что в реестре',
    /\| ключ ссылки \| id отчёта \| проверено \|/.test(out));
  ...
  check('ключ назначен своему отчёту, а не соседнему',
    !/\| hr-executive-detail-employee \| r-attendance-calendar \|/.test(out));
  ...
  // ПЕРМАЛИНК В СТАТЬЕ — НЕ ВТОРОЙ КЛЮЧ ОТЧЁТА, а адрес состояния дашборда.
  // Выдавать его за расхождение значит предлагать завести в мост строку,
  // которую валидатор оттуда же и выбрасывает.
  check('пермалинк из статьи расхождением не назван',
    !/--- РАСХОЖДЕНИЕ/.test(conflict));
  check('строка из каталога всё равно готова',
    /\| 35005 \| r-gitlab-activity \| /.test(conflict));
  check('а пермалинк назван словом и объяснён',
    /в статье пермалинк grp7GLNyp7V, в мост он не идёт/.test(conflict));

  // Настоящее расхождение — два РАЗНЫХ ключа дашборда на один отчёт:
  // у отчёта бывает и числовой, и слаг, и заводить надо обе строки.
  const twoKeys = runKeys(
    [R('r-hr-executive-report', 'urn:dd:reports:reports:report:1845')],
    [link('https://proteus.tcsbank.ru/superset/dashboard/1845/')]);
  check('два разных ключа — это расхождение', /--- РАСХОЖДЕНИЕ/.test(twoKeys));
  check('и сказано, что заводить надо ОБЕ строки',
    /Заводите ОБЕ строки/.test(twoKeys));
}
