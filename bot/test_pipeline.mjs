...
function runMaterials(plan, articles = null, dd = null, trigger = {}) {
  const $ = (name) => {
    ...
    if (name === 'Read article') {
      ...
      // ФИКСТУРА ОПИСЫВАЕТ СТАТЬИ РОУТЕРА, служебные добираются кодом.
      // С 01.09 код кладёт на каждый вопрос ещё три статьи — витрину
      // по умолчанию, словарь синонимов и фильтр активности, — и все
      // они идут ПЕРЕД выбором роутера. «Read article» отдаёт по элементу
      // на файл в порядке plan.files, и короткая фикстура молча съезжала
      // на служебные: результат, написанный про turnover.md, доставался
      // active-headcount.md. Тесты при этом краснели не там, где сломано.
      // Поэтому короткая фикстура дополняется СПЕРЕДИ: её элементы всегда
      // отвечают хвосту — тому, что назвал роутер.
      const pad = plan.files.length - articles.length;
      const head = Array.from({ length: Math.max(0, pad) },
        () => ({ content: b64('# служебная статья, добранная кодом') }));
      return { all: () => head.concat(articles).map((json) => ({ json })) };
    }
...
line('1а00. ЧТЕНИЕ ИЗ GITLAB ПОВТОРЯЕТСЯ ПРИ СБОЕ');
{
  ...
  // не прочитался, обращение ушло с «нет ответа». Один 503 не должен стоить
  // обращения. Ключи — стандартные настройки узла n8n, а не параметры.
  for (const name of ['Get a file', 'Read article']) {
    const n = core.nodes.find((x) => x.name === name);
    check(`${name}: повторяет при сбое`,
      n.retryOnFail === true && n.maxTries >= 2 && n.waitBetweenTries >= 1000);
    // Повтор не отменяет мягкого отказа: исчерпав попытки, узел всё равно
    // пропускает отказ дальше, а не роняет ядро.
    check(`${name}: и после повторов отказ пропускается, а не роняет`,
...
// ====================================================================== 1а-
line('1а-. РАЗВОРОТ СПИСКОВ переживает подмену элемента перед собой');
{
  // Промах 01.09 в двух актах, одна причина. Гейт «Need DD» читал $json —
  // ветка не шла. Гейт починили — ветка умерла НА УЗЕЛ ДАЛЬШЕ: splitOut
  // разворачивал `dd` того же $json, получал ноль элементов, и
  // «Call DD Lookup» опять не выполнялся. Симптом тот же, место другое.
  //
  // Здесь моделируется ровно это: до разворота доехал ответ GitLab по статье
  // (так ведёт себя n8n, когда нода между ними выключена — данные идут
  // НАСКВОЗЬ). План при этом в порядке, и разворот обязан брать список у него.
  const PLAN = {
    files: ['kb/tables/mdm-employee-structure-d.md'],
    dd: [{ urn: 'urn:dd:tables:greenplum:table:emart.mdm_employee_structure_d', hint: '' }],
  };
  const GITLAB = { file_name: 'mdm-employee-structure-d.md', content: 'base64…' };
  const run = (nodeName, input) => {
    const $ = (n) => {
      if (n === 'Plan') return { first: () => ({ json: PLAN }) };
      throw new Error('node not executed: ' + n);
    };
    return new Function('$', '$json', js(nodeName))($, input);
  };

  const dd = run('Split DD', GITLAB);
  check('метаданные разворачиваются из плана, а не из входного элемента',
    dd.length === 1 && /mdm_employee_structure_d/.test(dd[0].json.urn));
  const paths = run('Split paths', GITLAB);
  check('пути статей — тоже из плана', paths.length === 1);
  // Ключ обязан остаться `files`: «Read article» читает путь как $json.files,
  // и переименование тихо оборвало бы чтение статей.
  check('ключ пути не переименован', paths[0].json.files === PLAN.files[0]);

  // Пустой план — законный исход (вопрос не про поля), ноль элементов.
  const empty = (() => {
    const $ = (n) => {
      if (n === 'Plan') return { first: () => ({ json: { dd: [], files: [] } }) };
      throw new Error('not executed');
    };
    return new Function('$', '$json', js('Split DD'))($, {});
  })();
  check('пустой план даёт ноль элементов, а не падение', empty.length === 0);

  // И ни одного splitOut в ядре не осталось: они читают только входной
  // элемент, сослаться на производителя оттуда нечем.
  const splits = core.nodes.filter((n) => n.type.endsWith('splitOut')).map((n) => n.name);
  check(`splitOut в ядре не осталось${splits.length ? ': ' + splits.join(', ') : ''}`,
    splits.length === 0);
}
...
line('1а. ТРАССИРОВКА: один текст вместо шести открытых нод');
{
  ...
  // «НЕ ЗАПУСКАЛСЯ» отличается от «отработал пусто»: это разные поломки,
  // а по логу они выглядят одинаково. Перепись теперь по ВСЕМ узлам ядра,
  // поэтому проверяем и её полноту — раньше в отчёт попадали пять листьев,
  // и «Split DD», на котором всё и оборвалось, не показывался вовсе.
  check('невыполненные узлы названы невыполненными',
    /Ask pairs: НЕ ЗАПУСКАЛСЯ/.test(t) && /Check result: НЕ ЗАПУСКАЛСЯ/.test(t));
  check('перепись включает узлы разворота',
    /Split DD: /.test(t) && /Split paths: /.test(t));
  check('путь прогона показан', /\*\*Путь прогона:\*\*/.test(t));
...
// ====================================================================== 1б
line('1б. ВИТРИНА СОТРУДНИКОВ читается всегда, а не только на пустом плане');
{
  // Промах 01.09: «все сотрудники с подтверждённой инвалидностью». Роутер дал
  // домен и узкую статью — то есть НЕ промолчал, — но витрину не назвал,
  // и признак `disability_flg` искать было негде. Слова
  // «инвалидность» в таблице «Домены» нет и быть не может: перечислить
  // в алиасах все поля витрины нельзя.
  const p = runPlan(JSON.stringify({
    domains: ['meetings'],
    articles: ['kb/tables/calendar-event.md'],
    dd_urn: '', field_hint: '', no_question: false,
  }));
  check('витрина сотрудников прочитана, хотя домен её мастером не звал',
    p.files.includes('kb/tables/mdm-employee-structure-d.md'));
  check('добор витрины назван отдельно от мастеров',
    p.added_default_mart.includes('t-emp-structure'));
  // Порядок: мастера домена ведут, витрина идёт следом. Для вопроса про
  // встречи вести должен календарь, а не витрина сотрудников.
  check('мастер домена идёт ПЕРЕД витриной по умолчанию',
    p.files.indexOf('kb/tables/calendar-event.md')
      < p.files.indexOf('kb/tables/mdm-employee-structure-d.md'));
  // Признак «роутер промолчал» — не то же самое, что «витрину добрал код».
  // Пока это было одно поле, телеметрия показала бы сто процентов промахов.
  check('роутер не назван промолчавшим — он дал домен', p.router_empty_plan === false);
}

// ====================================================================== 1в
line('1в. РОУТЕР ПРОМОЛЧАЛ — витрина та же, но признак промаха поднят');
{
  const p = runPlan(JSON.stringify({
    domains: [], articles: [], dd_urn: '', field_hint: '', no_question: false,
  }));
  check('витрина всё равно прочитана',
    p.files.includes('kb/tables/mdm-employee-structure-d.md'));
  check('промах роутера назван', p.router_empty_plan === true);
}

// ====================================================================== 1г
line('1г. РЕПЛИКА БЕЗ ВОПРОСА — витрину не читаем');
{
  const p = runPlan(JSON.stringify({
    domains: [], articles: [], dd_urn: '', field_hint: '', no_question: true,
  }));
  check('на «спасибо» витрина не добирается',
    !p.files.includes('kb/tables/mdm-employee-structure-d.md'));
}
...
line('5. СЛОМАННЫЙ вывод роутера не роняет флоу и НЕ МОЛЧИТ');
{
  ...
  check('витрина по умолчанию добрана', p.added_default_mart.length === 1);
...
line('6. ЛИМИТ ОБЪЁМА: обрезанное названо, а не выброшено молча');
{
  ...
  // Кодом на каждом вопросе добираются три статьи — витрина по умолчанию,
  // словарь синонимов и фильтр активности, — поэтому из двадцати статей
  // роутера доезжают тринадцать, а не шестнадцать.
  check('обрезанное перечислено', p.dropped.length === 7);
...
line('8г. ЛИМИТ и ДУБЛИ объектов DD');
{
  ...
  // Роутер назвал семь объектов, код добрал восьмой — инвентарь витрины
  // по умолчанию. Срезано четыре.
  check('обрезанное перечислено', p.dd_dropped.length === 4);
  // И срезан ХВОСТ, а не основание: инвентарь витрины по умолчанию режется
  // потолком первым, если приписан в конец, — а он-то и обязан приехать.
  check('витрина по умолчанию потолком не срезана',
    p.dd[0].urn.endsWith('.mdm_employee_structure_d') &&
    !p.dd_dropped.some((u) => u.endsWith('.mdm_employee_structure_d')));
...
line('9б. УЗЕЛ DD НЕ ВЫПОЛНЯЛСЯ, хотя объекты запланированы — сбой конвейера');
{
  ...
  // Объектов не планировали — узел законно не выполнялся, это НЕ сбой.
  //
  // Случай сузился с 01.09, и это не оговорка теста: инвентарь витрины
  // по умолчанию добирается кодом на ЛЮБОМ вопросе, значит хотя бы один
  // объект запланирован всегда. Не выполнившаяся ветка DD на вопросе —
  // теперь сбой конвейера по определению, и это правильно: именно молчание
  // этой ветки стоило разбора 01.09. Остаётся реплика без вопроса —
  // там не читается вообще ничего.
  const p2 = runPlan(JSON.stringify({
    domains: [], articles: [], dd: [], no_question: true,
  }));
  check('на реплике без вопроса объектов не планируется', p2.dd.length === 0);
  const m2 = runMaterials(p2, [], null);
...
line('15. ВЫБОР РОУТЕРА отделён от добора мастеров');
{
  ...
  check('мастера всё равно добраны',
    mastersOnly.added_masters.length === 3);
  // Пять файлов: три мастера домена плюс словарь синонимов и фильтр
  // активности, которые едут на каждом вопросе. Витрина по умолчанию
  // четвёртой не становится — она же мастер headcount и уже прочитана.
  // Число пришпилено намеренно: молчаливое разрастание обязательного
  // чтения — расход на КАЖДОМ обращении, и замечать его должен тест.
  check('и сверх них — только то, что едет всегда',
    mastersOnly.files.length === 5);
...
line('24. ЭКСПЕРТ ПО ТЕМЕ: код находит, кого позвать, и не заменяет им ответ');
{
  ...
  // Единственный объект — инвентарь витрины по умолчанию, добранный кодом
  // на любом вопросе. МАРШРУТ не добавляет к нему ничего: «кто ведёт квоты»
  // решается таблицей «Маршруты», а не каталогом.
  check('и не добавляет объектов в DD сверх витрины по умолчанию',
    p.dd.length === 1 && p.dd_added_by_code.length === 1);
...
line('25. ИНВЕНТАРЬ ВИТРИН добирает код, а несобранный — называется');
{
  ...
  // Гейт остался, но НЕ для витрины по умолчанию: её инвентарь едет всегда.
  // Разбор 01.09: «есть ли поле про инвалидность» — вопрос, на который
  // отвечают данные, а не проза статьи; статья состав полей не перечисляет
  // и не должна. Гейт `isExport || isQueryHelp` на таком вопросе молчал,
  // инвентарь не запрашивался, и бот заявил, что признака в витринах нет.
  const ask = runPlan(ROUTER, REGISTRY, { question: 'Что такое текучесть?' });
  check('инвентарь витрины по умолчанию тянется и на обычном вопросе',
    ask.dd.length === 1 && ask.dd[0].urn === URN_TABLE);
  check('и она названа в tables_read',
    ask.tables_read.length === 1 && ask.tables_read[0].urn === URN_TABLE);
...
line('27. ПУСТОЙ ПЛАН РОУТЕРА — не «нет ответа», а витрина по умолчанию');
{
  ...
  check('умолчание выведено из реестра, а не вписано',
    p.added_default_mart.length === 1 &&
    p.added_default_mart[0] === 't-emp-structure');
  ...
  // Роутер назвал домен — витрина всё равно читается. Правило перевёрнуто
  // разбором 01.09: «все сотрудники с подтверждённой инвалидностью». Роутер
  // домен назвал, витрину — нет, а ответ лежал именно в ней. Витрина не
  // страховка на случай промаха, а основание по умолчанию; роутер добавляет
  // к ней частности. Тест держит уже это, а не прежнее «не добирается».
  const withDomain = runPlan(JSON.stringify({
    domains: ['movement'], articles: [], dd: [], no_question: false,
  }), REGISTRY, { question: 'сколько наняли' });
  // Витрина едет и здесь — но добирается она мастерами домена, а не шагом
  // умолчания: `added_default_mart` пуст ровно тогда, когда витрину уже
  // прочитали раньше. Держим ФАКТ ЧТЕНИЯ, а не то, чьей рукой она попала
  // в список: перепишется таблица мастеров — тест не должен краснеть зря.
  check('при названном домене витрина всё равно прочитана',
    withDomain.files.includes('kb/tables/mdm-employee-structure-d.md'));
...
line('28. ЗНАЧЕНИЯ ФИЛЬТРОВ: роутер их больше не выделяет');
{
  ...
  // Здесь же держится и разнообразие: примеры обязаны покрывать разные домены.
  {
    ...
    check(`примеры покрывают разные домены (${seenDomains.size})`,
      seenDomains.size >= 3);
    // МАСТЕРОВ КОД ДОБИРАЕТ САМ, как только назван домен. Пример, который
    // называет мастера возвращённого им же домена, учит тратить одно из шести
    // мест на уже прочитанное. Список мастеров берётся ИЗ РЕЕСТРА, а не
    // вписывается сюда: он меняется вместе с доменами — 02.09 их стало
    // четырнадцать вместо десяти, и вписанный список разъехался бы молча.
    const mastersOf = {};
    for (const l of reg.split('\n')) {
      const c = l.split('|').map((x) => x.trim());
      if (c.length === 5 && c[1] && !/^-+$/.test(c[1]) && c[1] !== 'домен') {
        mastersOf[c[1]] = c[3].split(',').map((x) => x.trim()).filter(Boolean);
      }
    }
    for (const b of blocks) {
      const covered = new Set(b.domains.flatMap((d) => mastersOf[d] || []));
      // Витрина сотрудников и словарь синонимов едут на КАЖДОМ вопросе,
      // независимо от домена, — они лишние в любом примере.
      for (const always of ['t-emp-structure', 'rc-field-synonyms']) covered.add(always);
      const waste = b.articles.filter((a) => covered.has(a));
      check(`пример [${b.domains.join(', ')}]: не тратит место на добираемое кодом`,
        waste.length === 0);
    }
  }
...
line('36. АКТИВНАЯ ЧИСЛЕННОСТЬ — УМОЛЧАНИЕ ЛЮБОГО ЗАПРОСА ПО СОТРУДНИКАМ');
{
  ...
  // С 01.09 добирается и на обычном вопросе. Гейт был `isQueryHelp ||
  // isExport`, то есть догадка о намерении по словам вопроса, — а цена
  // ошибки несимметрична: лишняя статья стоит абзац контекста, пропущенная
  // даёт ответ по неверной численности, и человек этого не видит.
  // «Все сотрудники с подтверждённой инвалидностью» не выгрузка и не просьба
  // про запрос — гейт молчал ровно там, где фильтр активности и решает,
  // кого считать сотрудником.
  check('на обычном вопросе правило работает тоже',
    ask('что такое текучесть').added_headcount.length === 1);
  // Но на реплике без вопроса — по-прежнему тишина: читать там нечего.
  const noQ = runPlan(JSON.stringify({
    domains: [], articles: [], dd: [], no_question: true,
  }), REGISTRY, { question: 'ага, спасибо' });
  check('на реплике без вопроса правило молчит',
    noQ.added_headcount.length === 0);
  ...
  // Ловится ФОРМУЛА, а не имя поля: `active_employee_flg` названо в узле
  // как пример лесов (поля механики, которые стоят почти в каждом черновике
  // и не считаются ответом по существу), и запрещать упоминание значило бы
  // запрещать объяснять код. Формула — это сравнение, вот его и нет.
  check('формулы метрики в узле Plan нет',
    !/active_employee_flg\s*=/.test(js('Plan')));
...
line('41б. СЛОВАРЬ СИНОНИМОВ добирается КОДОМ, а не по просьбе в промпте');
{
  ...
  // Гейт снят вместе с гейтом активной численности, и по той же причине:
  // он опирался на догадку «просят ли написать запрос», а мост «слово
  // заказчика → поле» нужен раньше — как только в вопросе появилось слово
  // заказчика. «Покраска» в вопросе «что такое покраска» не переводится
  // ничем другим, а гейт на нём молчал.
  const plain = runPlan(
    JSON.stringify({ domains: ['movement'], articles: [], dd: [], no_question: false }),
    REGISTRY, { question: 'что такое текучесть' },
  );
  check('на обычном вопросе словарь добирается тоже',
    plain.added_synonyms.includes('rc-field-synonyms'));
  // Граница — реплика без вопроса: там не читается вообще ничего.
  const noQ = runPlan(
    JSON.stringify({ domains: [], articles: [], dd: [], no_question: true }),
    REGISTRY, { question: 'ага, спасибо' },
  );
  check('на реплике без вопроса словарь молчит', !noQ.added_synonyms.length);
...
line('54. ИНВЕНТАРЬ ИЗ DD LOOKUP РАЗБИРАЕТСЯ ЯДРОМ ЦЕЛИКОМ');
{
  ...
  // Ломается тот формат, который шейпер ПЕЧАТАЕТ СЕЙЧАС: с 01.09 это
  // перечень по строкам, а не список через запятую. Подмена запятых
  // на прежнем тексте ничего не меняла бы — тест был бы зелёным всегда.
  const broken = meta.replace(NAMES.join('\n'), NAMES.join(' | '));
  ...
  const scan = (nodes) => {
    ...
    for (const n of nodes) {
      for (const l of codeLines(n.parameters.jsCode)) {
        ...
        // ОДНА строка — ОДИН заголовок, самый длинный из встреченных.
        // Заголовки вложены друг в друга: шейпер печатает и «ПОЛНЫЙ
        // ИНВЕНТАРЬ ПОЛЕЙ», и фразу «Это ПОЛНЫЙ состав витрины», так что
        // в `printed` попадает ещё и голое «ПОЛНЫЙ». Разбор строки при этом
        // один, и требовать заявки на каждый огрызок значит заводить записи
        // про разборы, которых нет: сетка станет шумной, а шумную сетку
        // глушат целиком. Разбирается самое частное совпадение — оно и есть
        // тот заголовок, на который написана регулярка.
        const head = [...printed].filter((h) => l.includes(h))
          .sort((a, b) => b.length - a.length)[0];
        if (head) hits.push({ node: n.name, head, l });
      }
    }
    return hits;
  };
