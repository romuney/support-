...
function valuesBlock() {
  ...
  // Потолки ПОКАЗА живут здесь, а не в SQL: запрос отдаёт по полю плоский
  ...
  for (const [f, g] of byField) {
    out.push('');
    out.push(`— ${f}:`);
    for (const r of g.hit.slice(0, maxHit)) out.push(`    «${r.val}» — ${r.cnt} строк`);
    if (g.hit.length > maxHit) {
      out.push(`    …и ещё ${g.hit.length - maxHit} совпавших значений, не показаны`);
    }
    if (g.sample.length) {
  ...
  // Поле упёрлось в потолок ЗАПРОСА — значит, из хранилища пришло не всё.
  // Это другое, чем потолок показа выше: там мы решили не печатать, здесь
  // мы просто не спрашивали. Молча обрезанный перечень читается как полный,
  // и «других значений нет» становится утверждением о непроверенном факте.
  const capped = [...byField.entries()]
    .filter(([, g]) => plan.values_limit &&
      g.hit.length + g.sample.length >= plan.values_limit)
    .map(([f]) => f);
  if (capped.length) {
    out.push(
      `  У полей ${capped.join(', ')} из хранилища пришли первые ` +
        `${plan.values_limit} значений — совпавшие подняты наверх сортировкой, ` +
...
const MAX_SAMPLE = 6;
// Строк на ПОЛЕ из хранилища. Совпавшие идут первыми по сортировке,
// поэтому потолок режет хвост словаря, а не искомое.
const MAX_ROWS = 60;
