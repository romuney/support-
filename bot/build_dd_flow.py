    ...
    if (matched.length) {
      ...
      out.push(
        `Показаны ТОЛЬКО поля, совпавшие с «${inp.search}» — остальные ` +
          ...
          'ни именами, ни описаниями. Но полный состав таблицы есть ниже: ' +
          'судить об отсутствии поля надо по НЁМ, а не по этому блоку.',
      );
      inventory(
    ...
    if (pick.wide_filter) {
      ...
      inventory(
        `Фильтр «${inp.search}» совпал с ${hit.length} полями из ${total} — ` +
          'он слишком широкий и ничего не сужает, поэтому описания ' +
          'не запрашивались. Выбери конкретное поле из списка ниже ' +
          'и вызови dd_lookup с ним.',
      );
    } else if (!hit.length) {
      inventory(
