...
function valuesBlock() {
  ...
  // Совпавшее и словарь поля печатаются РАЗДЕЛЬНО и подписаны по-разному.
  //
  // Раньше запрос возвращал только совпавшее, и промах слов давал ноль строк:
  // «таких значений нет» при живых данных. Теперь приходит и то, и другое,
  // но смешивать их нельзя — «нашлось по вашим словам» и «а вот как поле
  // называет свои значения вообще» это разные утверждения, и второе НЕ
  // является ответом на вопрос заказчика.
  const isYes = (v) => v === true || v === 'true' || v === 1 || v === '1';
  const byField = new Map();
  for (const r of rows) {
    const f = String(r.fld || '');
    if (!byField.has(f)) byField.set(f, { hit: [], sample: [] });
  ...
  out.push(
    anyHit
      ? `ЗНАЧЕНИЯ ПОЛЕЙ, найденные в данных по словам «${words}»:`
      : `ЗНАЧЕНИЯ ПОЛЕЙ: по словам «${words}» не совпало НИ ОДНО значение. ` +
        'Ниже — как эти поля называют свои значения на самом деле; ' +
        'скорее всего, заказчик называет то же самое другими словами.',
  );
  for (const [f, g] of byField) {
    out.push('');
    out.push(`— ${f}:`);
    for (const r of g.hit) out.push(`    «${r.val}» — ${r.cnt} строк`);
    if (g.sample.length) {
      out.push(g.hit.length
        ? '    прочие значения этого поля (для понимания, как оно устроено):'
        : '    самые частые значения этого поля:');
      for (const r of g.sample) out.push(`      «${r.val}» — ${r.cnt} строк`);
    }
  }
  out.push('');
  out.push(
    'Это РЕАЛЬНЫЕ значения из витрины' +
      ...
      ', а не догадка. Выбери подходящее сам или уточни у заказчика, если ' +
      'подходящих несколько — подставлять «похожее» нельзя, запрос ' +
      'выполнится и вернёт не те цифры.',
  );
  if (!anyHit) {
    out.push(
      '  Ни одно значение не совпало со словами заказчика — значит, ' +
        'НЕ УТВЕРЖДАЙ, что такого среза нет. Сопоставь его слова с этим ' +
        'списком сам, а если сопоставить нечем — так и спроси у заказчика, ' +
        'какое из значений он имеет в виду.',
    );
  }
  if ((plan.values_skipped || []).length) {
    out.push(
  ...
  // Потолок теперь НА ПОЛЕ, поэтому и обрезка считается по полю: общий
  // лимит давал одному полю вытеснить остальные, а его переполнение
  // ничего не говорило о том, у какого поля список неполон.
  const capped = [...byField.entries()]
    .filter(([, g]) => plan.values_limit && g.hit.length >= plan.values_limit)
    .map(([f]) => f);
  if (capped.length) {
    out.push(
      `  У полей ${capped.join(', ')} показаны первые ${plan.values_limit} ` +
        'совпавших значений по убыванию числа строк — список ОБРЕЗАН. ' +
        'Судить по нему об отсутствии другого значения нельзя.',
    );
  }
  out.push(
...
// несопоставимы. Совпавших показываем много (их и просили), несовпавших —
// горсть, просто чтобы было видно, КАК поле называет свои значения.
const MAX_MATCHED = 25;
const MAX_SAMPLE = 6;
...
// Ветвь на поле: словарь значений с частотой. Фильтра по словам здесь нет
// намеренно — он не экономит скан, а только теряет ответ (см. ниже).
...
// ЗАПРОС ВОЗВРАЩАЕТ И СОВПАВШЕЕ, И СЛОВАРЬ ПОЛЯ — за одну попытку.
//
// Раньше фильтр `LIKE` стоял в WHERE, и при промахе слов приходило ноль
// строк: бот говорил «таких значений нет», хотя данные есть, а второй
// попытки в конвейере нет и быть не должно.
//
// Ошибка была в расчёте стоимости. `GROUP BY` и так сканирует весь срез
// витрины, а WHERE по LIKE режет только ВЫДАЧУ, не скан. Значит вытащить
// весь словарь значений поля стоит ровно столько же, сколько
// отфильтрованный, — и ретрай не нужен: первая же попытка может отдать
// и то, что совпало, и то, как поле называет свои значения вообще.
//
// Отсюда форма: группировка без фильтра, признак `matched` считается
// НАД ней (поэтому список LIKE пишется один раз, а не в каждую ветвь),
// и потолки по разделам — совпавших много, несовпавших горсть.
const union = parts.join('\nUNION ALL\n');
const matchExpr = like.map((l) => `lower(val) LIKE ${l}`).join(' OR ') || 'false';

return [{ json: {
  values_sql:
    `SELECT fld, val, cnt, matched FROM (\n` +
    `  SELECT fld, val, cnt, matched,\n` +
    `         row_number() OVER (PARTITION BY fld, matched ORDER BY cnt DESC) AS rn\n` +
    `  FROM (\n` +
    `    SELECT fld, val, cnt, (${matchExpr}) AS matched\n` +
    `    FROM (\n${union}\n    ) u\n` +
    `  ) m\n` +
    `) r\n` +
    `WHERE (matched AND rn <= ${MAX_MATCHED}) OR (NOT matched AND rn <= ${MAX_SAMPLE})\n` +
    `ORDER BY fld, matched DESC, cnt DESC`,
  values_limit: MAX_MATCHED,
